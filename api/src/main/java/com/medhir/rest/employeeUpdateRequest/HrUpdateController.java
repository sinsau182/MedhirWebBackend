...
public class HrUpdateController {
    ...
    private List<UpdateFieldComparison> compareFields(EmployeeModel employee, EmployeeUpdateRequest updateRequest) {
        ...
        if (updateRequest.getEmailPersonal() != null && !updateRequest.getEmailPersonal().equals(employee.getEmailPersonal())) {
        ...
        if (updateRequest.getAlternatePhone() != null && !updateRequest.getAlternatePhone().equals(employee.getAlternatePhone())) {
            changes.add(new UpdateFieldComparison("alternatePhone",
                    employee.getAlternatePhone(), updateRequest.getAlternatePhone()));
        }

        if (updateRequest.getCurrentAddress() != null && !updateRequest.getCurrentAddress().equals(employee.getCurrentAddress())) {
            changes.add(new UpdateFieldComparison("Current Address", 
                employee.getCurrentAddress(), updateRequest.getCurrentAddress()));
        }

        if (updateRequest.getPermanentAddress() != null && !updateRequest.getPermanentAddress().equals(employee.getPermanentAddress())) {
        ...
        if (updateRequest.getProfileImgUrl() != null) {
            changes.add(new UpdateFieldComparison("Profile Image", 
                "Previous Image", "New Image Uploaded"));
        }
